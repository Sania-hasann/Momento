...
class Retrieval:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized Retrieval with custom splitters and model: {Config.EMBEDDING_MODEL}"
        )
    ...
    def vector_search(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if results.nodes:
                for i, node in enumerate(results.nodes):
                    score = (
                        ...
                        if results.similarities and i < len(results.similarities)
                        else 1.0
                    )
            ...
            logger.info(
                f"Retrieved {len(documents)} documents using improved vector search"
            )
            return documents
    ...
    def get_document_by_node_id(self, node_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            # Since node IDs don't match database IDs, we need to find the document
            # by searching for it in the search results and then querying the database
            # by content or metadata

            # First, try to find the document in recent search results
            # This is a workaround for the ID mismatch issue
            search_results = self.vector_search("", k=100)  # Get many results
    ...
    def true_hybrid_search(
        self, query: str, k: int = 5, vector_weight: float = 0.7
    ) -> List[Dict[str, Any]]:
        """
        Perform true hybrid search combining vector similarity and full-text search with improved results.
        ...
        try:
            ...
            # 4. Return top-k results with final deduplication
            final_results = self._deduplicate_results(combined_results, k)
    ...
    def _postgres_full_text_search(
        ...
        try:
            ...
            search_conditions.append(
                "to_tsvector('english', text) @@ websearch_to_tsquery('english', %s)"
            )
            search_params.append(query)
            ...
            all_params = (
                [query, f"%{query}%"] + search_params + [k * 2]
            ...
            for row in results:
                ...
                if content_hash not in seen_contents:
                    ...
                    exact_bonus = (
                        float(row["exact_match_bonus"])
                        if row["exact_match_bonus"]
                        else 0.0
                    )
            ...
            logger.info(
                f"Retrieved {len(documents)} documents using improved full-text search"
            )
            return documents
    ...
    def _normalize_scores(self, results: List[Dict], score_field: str) -> List[Dict]:
        """
        Normalize scores to 0-1 range for better differentiation.
        ...
        for result in results:
            if score_field in result and result[score_field] is not None:
                if score_range == 0:
                    ...
                    result[f"normalized_{score_field}"] = (
                        result[score_field] - min_score
                    ) / score_range
    ...
    def _combine_search_results(
                ...
                combined_doc.update(
                    {
                        ...
                        "vector_score": vector_doc.get(
                            "normalized_similarity", vector_doc["similarity"]
                        ),
                ...
                combined_doc.update(
                    {
                        ...
                        "text_score": text_doc.get(
                            "normalized_text_score", text_doc["text_score"]
                        ),
        ...
        # Apply deduplication
        combined_results = self._deduplicate_results(
            combined_results, len(combined_results)
        )

        logger.info(
            f"Combined {len(combined_results)} documents with vector_weight={vector_weight}"
        )
