...
class DocumentIngestion:
    ...
    def __init__(self):
        ...
        # Initialize index # TODO learn more about this
        self.index = VectorStoreIndex.from_vector_store(
            vector_store=self.vector_store, service_context=self.service_context
        )

        logger.info(
            f"Initialized DocumentIngestion with custom splitters and model: {Config.EMBEDDING_MODEL}"
        )

    def _get_node_parser_for_content(
    ...
    def _create_service_context_with_parser(self, node_parser) -> ServiceContext:
        ...
        return ServiceContext.from_defaults(
            embed_model=self.embedding_model, node_parser=node_parser, llm=None
        )
    ...
    def ingest_text(self, text: str, metadata: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            node_parser = self._get_node_parser_for_content(
                text, metadata.get("file_path") if metadata else None
            )
            logger.info(10 * "*")
            logger.info(node_parser)
            logger.info(10 * "*")
            # Create LlamaIndex Document with the generated ID
            document = Document(text=text, metadata=metadata or {}, id_=doc_id)
            ...
            logger.info(
                f"Ingested text document with ID: {doc_id} using {node_parser.__class__.__name__}"
            )
            return doc_id
    ...
    def ingest_markdown_directory(
        self, data_dir: str, recursive: bool = True
    ) -> List[str]:
        """
        ...
        logger.info(
            f"Starting markdown ingestion from directory: {data_dir} using custom markdown splitter"
        )
