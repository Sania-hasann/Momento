...
def analyze_search_results(results, search_type, query):
    ...
    # Enhanced analysis
    score_variance = max(score_range) - min(score_range) if score_range else 0
    avg_score = sum(score_range) / len(score_range) if score_range else 0
    
    logger.info(f"      Score variance: {score_variance:.3f}")
    ...
    # Check score distribution with improved thresholds
    if score_variance < 0.05:
        logger.warning(f"      ⚠️  POOR SCORE DIFFERENTIATION: Score variance too low ({score_variance:.3f})!")
    elif score_variance < 0.1:
        logger.info(f"      ℹ️  MODERATE SCORE DIFFERENTIATION: Score variance is {score_variance:.3f}")
    else:
        logger.info(f"      ✅ GOOD SCORE DIFFERENTIATION: Score variance is {score_variance:.3f}")
    
    # Check diversity with improved thresholds
    if len(unique_categories) < 2:
        logger.warning(f"      ⚠️  LOW DIVERSITY: Only {len(unique_categories)} category found!")
    elif len(unique_categories) < 3:
        logger.info(f"      ℹ️  MODERATE DIVERSITY: {len(unique_categories)} categories found")
    else:
        logger.info(f"      ✅ GOOD DIVERSITY: {len(unique_categories)} categories found")
    
    # Show top 3 unique results with enhanced formatting
    ...
    for i, result in enumerate(results, 1):
        ...
        if content_preview not in seen_contents and count < 3:
            ...
            # Enhanced result display with score quality indicator
            score_quality = "🟢" if score > 0.7 else "🟡" if score > 0.4 else "🔴"
            logger.info(f"      {count}. {score_quality} [{category}/{topic}/{difficulty}] Score: {score:.3f}")
