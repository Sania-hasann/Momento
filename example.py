...
def analyze_search_results(results, search_type, query):
    ...
    for result in results:
        ...
        unique_categories.add(
            str(result.get("metadata", {}).get("category", "unknown"))
        )
    ...
    if len(unique_contents) < len(results):
        logger.warning(
            f"      ⚠️  DUPLICATE ISSUE: {len(results) - len(unique_contents)} duplicate results detected!"
        )
    ...
    if len(unique_categories) < 2:
        logger.warning(
            f"      ⚠️  LOW DIVERSITY: Only {len(unique_categories)} category found!"
        )
    ...
    for i, result in enumerate(results, 1):
        ...
        if content_preview not in seen_contents and count < 3:
            ...
            logger.info(
                f"      {count}. [{category}/{topic}/{difficulty}] Score: {score:.3f} | {result['content'][:80]}..."
            )
